...
def setup_platform(hass, conf, add_entities, discovery_info=None):
    ...
    add_entities([XploraSensor(hass, description, controller, scan_interval, start_time, _types) for description in SENSOR_TYPES], True)

class XploraSensor(SensorEntity):

    def __init__(self, hass, description: SensorEntityDescription, controller, scan_interval, start_time, _types):
